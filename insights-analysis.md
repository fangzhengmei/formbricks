...
| 指标 | 欢迎卡 = 关闭 | 欢迎卡 = 开启 |
|------|--------------|--------------|
| **首题曝光数** | = `displayCount`（问卷总展示数，展示口径强制对齐） | = 真实"看到或交互"首题的人数（响应数据口径） |
...
| 输入值 | 数值 | 说明 |
|--------|------|------|
| `displayCount`（总展示） | 1000 人 | 埋点统计的问卷展示次数 |
| 首题真实曝光（wasElementSeen = true） | 850 人 | 其中 700 人答了题，150 人有停留但没答 |

**计算过程**：
1. 首题流失数 = 1000 - 850 = **150 人**（展示了但完全没看到首题的人）
2. 首题流失率 = 150 / 1000 = **15%**
3. 首题曝光数（最终输出） = **1000**（回填总展示口径）

**解读**：1000 次展示中，15% 的用户完全没看到第一题就离开了（可能是页面加载慢、闪退、立即关闭等原因）
...
|--------|------|------|
| `displayCount`（总展示） | 1000 人 | 埋点统计的问卷展示次数 |
| 首题真实曝光（wasElementSeen = true） | 600 人 | 400 人看了欢迎页就走了，没进入首题 |
| 首题流失（看到首题但没继续） | 60 人 |

**计算过程**：
1. 首题流失数 = **60 人**（看到首题但没继续的人）
2. 首题流失率 = 60 / 600 = **10%**
3. 首题曝光数（最终输出） = **600**（真实看到首题的人数）

**解读**：进入第一题的 600 人中，10% 的人在首题流失；另外 40% 的展示在欢迎页阶段就流失了（这部分流失不在首题漏斗中体现）
