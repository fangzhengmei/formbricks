...
IndexedDB 进度清除是独立的逻辑，与 SurveyState.clear() 没有直接关系。根据源码，触发清除的场景分为 **两个阶段**：

**触发阶段分类**：

| 阶段 | 场景 | 触发目的 |
|------|------|----------|
| **恢复阶段**（组件挂载时） | 进度过期、问卷已完成且无待发送响应、问卷结构变化 | 清理无效数据或避免恢复无效 UI 进度 |
| **同步阶段**（网络恢复后） | 离线响应同步成功 | 会话完成清理 |

**清除触发时机详细列表**：

| 场景 | 代码位置 | 所属阶段 | 说明（持久化层 vs 内存层） |
|------|----------|----------|---------------------------|
| **进度过期** | `survey.tsx:448-452` | 恢复阶段 | 持久化层：`clearSurveyProgress()` 完全删除 IndexedDB 记录<br>内存层：保持初始状态，从头开始 |
| **问卷已完成且无待发送响应** | `survey.tsx:460-468` | 恢复阶段 | 持久化层：`clearSurveyProgress()` 完全删除 IndexedDB 记录<br>内存层：保持初始状态，从头开始 |
| **问卷结构变化** | `survey.tsx:523-526` | 恢复阶段 | **两步操作：**<br>1. 持久化层：`clearSurveyProgress()` 完全删除 IndexedDB 记录<br>2. 内存层：用**已读取到内存中的**快照恢复 SurveyState |
| **离线响应同步成功** | `survey.tsx:585-590` | 同步阶段 | 持久化层：`clearSurveyProgress()` 完全删除 IndexedDB 记录<br>内存层：不受影响 |

**关键注意**：
1. `clearSurveyProgress()` 函数的行为是统一的——**总是完全删除** IndexedDB 中 `surveyProgress` 对象存储的该问卷记录
2. 不同场景的差异在于：
   - **触发阶段**：恢复阶段（组件挂载时）或同步阶段（网络恢复后）
   - **内存层是否恢复**：只有"问卷结构变化"场景会用已读取的快照恢复内存态
   - **触发目的**：过期/已完成是为了清理，结构变化是为了避免恢复无效 UI 进度，同步成功是为了会话完成清理
...
| **IndexedDB 清除** | 分两个阶段：<br>1. **恢复阶段**（组件挂载时）：发现进度无效<br>2. **同步阶段**（网络恢复后）：离线响应同步成功 | 持久化层：`clearSurveyProgress()` **总是完全删除** `surveyProgress` 中的该问卷记录<br>内存层：取决于场景，可能恢复也可能不恢复 |

**关键澄清**：
1. `clearSurveyProgress()` 函数的行为是统一的——**总是完全删除** IndexedDB 中 `surveyProgress` 对象存储的该问卷记录，不存在"部分清除"
2. 触发清除的场景分两个阶段：
   - **恢复阶段（3 个场景）**：进度过期、问卷已完成且无待发送响应、问卷结构变化
   - **同步阶段（1 个场景）**：离线响应同步成功
3. 不同场景的差异在于内存层是否恢复：
   - **过期/已完成场景**：清除后直接返回，内存层保持初始状态
   - **结构变化场景**：清除后用**已读取到内存中的**快照恢复内存层
   - **同步成功场景**：清除时问卷可能已完成，内存层不受影响
